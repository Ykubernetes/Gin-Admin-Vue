package core

import (
	"bytes"
	"fmt"
	"gitee.com/go-server/global"
	"github.com/sirupsen/logrus"
	"os"
	"path"
)

// 颜色
const (
	red    = 31
	yellow = 33
	blue   = 36
	gray   = 37
)

type LogFormatter struct {
}

func (l *LogFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	var levelColor int
	switch entry.Level {
	case logrus.DebugLevel, logrus.TraceLevel:
		levelColor = gray
	case logrus.WarnLevel:
		levelColor = yellow
	case logrus.ErrorLevel, logrus.FatalLevel, logrus.PanicLevel:
		levelColor = red
	default:
		levelColor = blue
	}
	var b *bytes.Buffer
	if entry.Buffer != nil {
		b = entry.Buffer
	} else {
		b = &bytes.Buffer{}
	}
	// 自定义日期格式
	timestamp := entry.Time.Format("2006-01-02 15:04:05")
	if entry.HasCaller() {
		// 自定义文件路径
		funcVal := entry.Caller.Function
		fileVal := fmt.Sprintf("%s:%d", path.Base(entry.Caller.File), entry.Caller.Line)
		// 自定义输出格式
		fmt.Fprintf(b, "%s [%s] \x1b[%dm[%s]\x1b[0m %s %s %s\n", global.Config.Logger.Prefix, timestamp, levelColor, entry.Level, fileVal, funcVal, entry.Message)
	} else {
		fmt.Fprintf(b, "%s [%s] \x1b[%dm[%s]\x1b[0m %s\n", global.Config.Logger.Prefix, timestamp, levelColor, entry.Level, entry.Message)
	}
	return b.Bytes(), nil
}

func InitLogger() *logrus.Logger {
	mLog := logrus.New() // 新建一个实例

	// 文件路径 日志
	//logPath := global.Config.Logger.Director                      // 日志存放路径
	//src, err := os.OpenFile(logPath, os.O_RDWR|os.O_CREATE, 0755) // 日志文件目录创建
	//if err != nil {
	//	log.Fatalf("日志文件创建失败", err)
	//	mLog.SetOutput(os.Stdout)
	//}
	//mLog.SetOutput(src) // 设置输出类型

	mLog.SetOutput(os.Stdout) // 终端输出日志
	// mLog.SetReportCaller(true)         // 开启返回函数名和行号
	mLog.SetReportCaller(global.Config.Logger.ShowLine) // 开启返回函数名和行号
	mLog.SetFormatter(&LogFormatter{})                  // 设置自定义的formatter
	//mLog.SetLevel(logrus.DebugLevel)   // 设置最低的level
	level, err := logrus.ParseLevel(global.Config.Logger.Level) //将配置文件中的level通过parselevel转换一下
	if err != nil {
		level = logrus.InfoLevel
	}
	mLog.SetLevel(level)
	return mLog
}

func InitDefaultLogger() {
	// 全局Log
	logrus.SetOutput(os.Stdout)
	logrus.SetReportCaller(global.Config.Logger.ShowLine)
	logrus.SetFormatter(&LogFormatter{})
	level, err := logrus.ParseLevel(global.Config.Logger.Level) //将配置文件中的level通过parselevel转换一下
	if err != nil {
		level = logrus.InfoLevel
	}
	logrus.SetLevel(level)
}
